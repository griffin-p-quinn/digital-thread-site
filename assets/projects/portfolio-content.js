(function portfolioContentModel() {
  ...
  const PROJECTS = Object.freeze([
    defineProject({
      ...
      griffinsRole: 'Griffin shaped the workflow and built the React 19 and Siemens iX interface, state model, sample work orders, and guided assessment-to-disposition path.',
      roleOfGenAI: 'Generative AI acted as an implementation partner while Griffin supplied the maintenance domain flow, interface constraints, and acceptance judgment.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin assembled the Node-RED flow, mapped the nine Industrial IoT signals, configured the OPC UA-to-MQTT path, and built the Mendix-facing operating view.',
      roleOfGenAI: 'Generative AI accelerated the flow and application implementation while Griffin defined the signal semantics, integration boundaries, and expected operating states.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin built the .NET WPF shell, WebView2-hosted React interface, repository scanner, SQLite metadata store, and bounded process-output view.',
      roleOfGenAI: 'Generative AI helped implement the desktop and web layers from Griffin’s launch workflow and process-control constraints.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin chose the signal-to-action narrative, composed the locally bundled Siemens iX components, and built the navigable concept.',
      roleOfGenAI: 'Generative AI helped translate the scoped workflow into interface variations; Griffin selected the scenario, information order, and final design.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin built and packaged the extension JAR, provider-profile flow, MCP server loading, in-studio chat surface, and tool trace.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin prepared the demonstration, used AutoModel to build the predictive model, published the scoring process to AI Hub, and built the engineering-change cost dashboard.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin defined the connected change story, modeled its evidence trail, and built the command-center interface and interactive traversal.',
      roleOfGenAI: 'Generative AI helped implement and iterate the graph-driven interface while Griffin supplied the lifecycle relationships and decided what evidence the story must expose.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin built the Mendix pluggable widget around PLMVisWeb and JTReader, added URL and document inputs, exposed viewer settings, and implemented state write-back.',
      roleOfGenAI: 'Generative AI accelerated widget implementation and debugging while Griffin defined the Mendix contract, viewer behavior, and integration boundaries.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin organized the source material, defined the guide structure and review path, and built the python-docx generator.',
      roleOfGenAI: 'Generative AI helped extract and structure draft instructions from source material; Griffin reviewed the steps and controlled the generated documents.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin attempted the migration, collected the failure evidence, compared unsuccessful approaches, wrote the guardrails, and documented the reset plan.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin framed the decision, assembled the ten-slide comparison, and made the integration paths legible for a presales audience.',
      roleOfGenAI: 'Generative AI helped organize and render the presentation while Griffin supplied the MES context and reviewed the architectural distinctions.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin wrote and checked the conditional expressions and aligned their outputs with the intended demonstration products.',
      roleOfGenAI: 'Generative AI helped draft and review the repetitive expression logic; Griffin controlled the conditions and final values.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin built the initial OAuth client, separate token persistence, refresh path, and read-only example routes.',
      roleOfGenAI: 'Generative AI assisted implementation of the first broker while Griffin set the local-only and read-only boundaries.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin designed and packaged the broker, implemented both authentication paths, automatic refresh, CLI access, and sanitized read-only routes.',
      roleOfGenAI: 'Generative AI helped implement and test the broker clients; Griffin constrained the public examples to read-only behavior and separated credentials from project code.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin composed the Mendix experience and built the three custom pluggable-widget surfaces that carry the visual and reasoning story.',
      roleOfGenAI: 'Generative AI helped build the widget surfaces and graph-driven interaction while Griffin supplied the risk narrative, integration shape, and review criteria.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin built the parsing and serving workflow and organized the API and guide content into a searchable local reference.',
      roleOfGenAI: 'The searchable corpus gave coding agents grounded SDK context; Griffin controlled the source material and used it while building viewer integrations.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin created the paired fixtures, kept their schemas aligned, and prepared them for the Mendix connector demonstration.',
      roleOfGenAI: 'Generative AI helped generate and compare fixture setup; Griffin reviewed the schema and kept the records synthetic.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin retained and organized the NX and faceted artifacts so they could be inspected in the surrounding automation work.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin built the widget inputs for product, configuration, and revision rules, embedded the BOM view, and implemented UID, item, and revision write-back.',
      roleOfGenAI: 'Generative AI accelerated the widget bridge implementation while Griffin defined the Teamcenter inputs and Mendix selection contract.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin built the React and Three.js production view and paired it with LangGraph and ReAct diagnostic, recovery, and simulation tools in scripted and live modes.',
      roleOfGenAI: 'The agent interprets the visible manufacturing state and calls explicit diagnosis, recovery, and simulation tools; Griffin designed the tool sequence and kept the evidence visible.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin ran the IEVD and IEM virtual machines on his workstation, maintained their recoverable Hyper-V state, and used them to support the team demonstration.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin built model discovery, task-specific runners, result views, and HTML findings across the different benchmark surfaces.',
      roleOfGenAI: 'Multiple models are the subjects of the experiment; Griffin designed the task surfaces and kept their outputs separate instead of forcing one universal ranking.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin built the desktop workspace, provider and prompt flow, structured report assembly, local history, and packaged application.',
      roleOfGenAI: 'Gemini performs the requested video analysis; Griffin designed the request and report contract and the local workflow that makes each run inspectable.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin designed the story model and built the 3D map, 2D journey, draft editor, persistence, reordering, and publication flow.',
      roleOfGenAI: 'Generative AI helped implement the full-stack experience while Griffin supplied the lifecycle narrative model and made the authoring decisions explicit.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin assembled the PowerShell setup, inventory, audit, inspection, and measurement workflow around his demonstration-router needs.',
      roleOfGenAI: 'Generative AI helped implement and organize the checks while Griffin defined the safe target, expected services, and review sequence.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin codified the layout constraints, built the fixed HTML template and PDF pipeline, and used visual review to keep the output on one page.',
      roleOfGenAI: 'Generative AI helped draft and fit content within the template; Griffin controlled hierarchy, claims, page limits, and visual acceptance.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin wrote the brief, directed Codex and Sol, constrained the intended game loop, ran the build, and judged the resulting systems and playability.',
      roleOfGenAI: 'Codex and Sol produced and iterated the C++ and project implementation under Griffin’s prompts, corrections, and acceptance decisions.',
      ...
      validation: 'Griffin built and played the resulting slice and compared observed behavior with the brief rather than assigning an invented numerical score.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin designed the safety model and built the TypeScript/Node MCP server, native control center, packaging path, and interdependent capability gates.',
      roleOfGenAI: 'A coding agent uses the exposed local tools only when their capabilities are enabled; Griffin decides the authority model and allowed workspace.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin modeled the workbook data as a graph, built the local ingestion and API, designed the iX graph browser, and connected grounded chat.',
      roleOfGenAI: 'The assistant answers against retrieved graph context; Griffin defined the graph structure and kept the supporting nodes visible for review.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin designed the evidence model, merged the earlier goal draft into it, and built the organizer and presentation view.',
      roleOfGenAI: 'Generative AI helped structure and summarize evidence; Griffin selected the records, protected private details, and made readiness judgments.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin set the benchmark brief, directed Codex and Sol, reviewed builds, corrected the implementation, and judged what was playable and what remained weak.',
      roleOfGenAI: 'Codex and Sol produced and iterated the Godot implementation under Griffin’s direction, making their successes and missed requirements the subject of the project.',
      ...
      validation: 'Griffin ran and played the packaged game against the brief and recorded qualitative successes and shortcomings without inventing benchmark scores.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin scoped the test, directed the implementation, built the interaction and visual model with the agent, and insisted on explicit synthetic-data boundaries.',
      roleOfGenAI: 'A coding agent helped research terminology and produce the simulator; Griffin treated every scientific claim as provisional and constrained the output to an interface experiment.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin defined the outline and content contracts and built the Node-based PowerPoint assembly workflow.',
      roleOfGenAI: 'Generative AI helps draft and organize slide content within the supplied structure; Griffin reviews facts, emphasis, and the final visual sequence.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin traced the NXOpen operation patterns, designed the typed tool contracts, built the FastMCP server, and assembled inspectable CAD and analysis evidence.',
      roleOfGenAI: 'An AI client can plan and call the explicit NX tools; Griffin supplies the CAD intent, parameter boundaries, runtime environment, and review of generated geometry.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin built the paged Mendix AGS REST ingestion, RDF transformation, SPARQL representation, and visible-edge validation workflow.',
      roleOfGenAI: 'Generative AI helped implement and inspect the transformation pipeline; Griffin defined the Teamcenter-to-ontology mapping and checked the resulting relationships.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin implemented the custom connector and adapted the provider path between Capital Copilot and locally hosted or open-weight models.',
      roleOfGenAI: 'The connected open-weight model supplies the assistant responses; Griffin built the integration and evaluated whether that provider path worked in the engineering-assistant surface.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin chose the narrow read-only use case, designed the typed flight tools, and built the FastMCP-to-REST adapter.',
      roleOfGenAI: 'An AI client discovers and calls the flight tools; Griffin kept the surface compact enough to study tool descriptions and results directly.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin explored the retrieval shape and retained the implementation fragment without promoting it beyond the evidence available.',
      roleOfGenAI: 'A language model consumes retrieved graph context; Griffin kept the concept bounded and chose not to present an unsupported performance conclusion.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin built the local MCP wrapper, selected the REST operations to expose, and translated their request and response shapes into typed tools.',
      roleOfGenAI: 'The AI client discovers and invokes the tools; Griffin controls which existing Capital REST operations are represented and how their parameters are described.',
      ...
      limitations: 'Griffin did not create Capital’s API, and the portfolio makes no unsupported claim about write governance, security controls, or production safety.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin built the polling connector, mapped Mendix configurations into NX templates and expressions, automated exports, and patched results back to the app.',
      roleOfGenAI: 'Generative AI accelerated connector and NXOpen implementation while Griffin supplied the parametric model rules, export contract, and result checks.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin built the reporting dashboard and its saved-query and SOA hydration flow, then shaped the KPI, workflow, and impact views.',
      roleOfGenAI: 'Generative AI accelerated implementation and helped organize the reporting surfaces; Griffin defined the Teamcenter queries, evidence fields, and review logic.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin modeled one connected event, assembled its system-specific artifacts, and built the staged narrative and traversal.',
      roleOfGenAI: 'Generative AI helped implement the story presentation while Griffin supplied the cross-system relationships and decided what evidence belonged in each step.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin organized the two reference implementations and retained their contrasting tool and authority boundaries as lineage.',
      roleOfGenAI: 'AI clients consume both tool surfaces; Griffin used the comparison to refine descriptions and capability boundaries in later MCP projects.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin studied two C# journals for simulation setup and algorithmic-feature logic, documented their ordered calls, and identified what needed parameterization.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin distilled his design process into the skill’s discovery, direction, implementation, responsive review, and critique instructions.',
      roleOfGenAI: 'The coding agent follows the skill as an operating guide; Griffin authored the constraints and examples that steer its design decisions.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin designed the read-only node and edge contract, implemented search and ingestion endpoints, and added request IDs and system logging for diagnosis.',
      roleOfGenAI: 'Generative AI helped implement the API and graph consumers; Griffin fixed the read-only boundary and reviewed the Teamcenter semantics.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin directed the research, set citation and uncertainty rules, reviewed the evidence, and built the HTML and PDF presentation.',
      roleOfGenAI: 'Codex helped locate, organize, and present candidate evidence; Griffin decided which relationships were supported and which remained unresolved.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin created the minimum, stretch, and evidence structure and later consolidated it into the more complete system.',
      roleOfGenAI: 'Generative AI helped structure draft goal language; Griffin selected evidence and chose to merge rather than publicly duplicate the work.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin documented the observed operations and built small client experiments across the application, repository, table, and flow surfaces.',
      roleOfGenAI: 'Generative AI assisted API exploration and client drafting; Griffin kept unsupported behavior and operational risk explicit.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin took the existing connector JAR, added the open-weight model path, integrated it with the AI Studio connection workflow, and checked the connection behavior.',
      roleOfGenAI: 'The open-weight language model is the newly reachable runtime; Griffin extended the connector rather than building the original JAR or the separate MCP agent extension.',
      ...
      limitations: 'The original connector predates Griffin’s work, the public portfolio cannot include configured model endpoints, and this project is distinct from MCP agent behavior.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin designed the opportunity-matching workflow, connected the Microsoft 365 and internal Salesforce context, and built the CSV review and adjustment surface.',
      roleOfGenAI: 'Microsoft 365 Copilot reasons over calendar, email, and WorkIQ context to propose hours; the Salesforce agent supplies opportunity context and Griffin reviews every estimate.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin built and packaged the widget collection, identified the recurring constraints, and authored the reusable specifications and skills shared with other Mendix builders.',
      roleOfGenAI: 'Coding agents helped implement widgets from the captured contracts; Griffin turned repeated failures and checks into explicit instructions and reviewed every packaged result.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin supplied the fleet-control brief, directed the generated interface, and judged its information hierarchy, interaction, and visual coherence.',
      roleOfGenAI: 'A coding agent produced and iterated the single-file interface; Griffin constrained the scenario and made the final design decisions.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin designed the comparison workflow, built the deal and MapLibre surfaces, connected the tool calls, and made the recommendation rationale visible.',
      roleOfGenAI: 'The model calls scoped comparison tools and explains its recommendation; Griffin controls the candidate data, tool results, and presentation of the reasoning.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin defined the multi-domain interface question and built the globe, layers, entity inspection, camera controls, and synthetic data adapters.',
      roleOfGenAI: 'Generative AI helped implement and iterate the Cesium interface while Griffin set the source labels, interaction rules, and explicit fixture boundary.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin set the visual prompt, directed the implementation, and kept the experiment intentionally small.',
      roleOfGenAI: 'A coding agent helped produce the animation from the compact brief; Griffin selected the final motion and composition.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin assembled the custom-directory and startup deployment, wired the managed DLL load, and added diagnostic logging.',
      roleOfGenAI: 'The configuration enables the runtime used by GenAI-assisted NX tooling; Griffin defined and verified the host setup rather than presenting it as a standalone agent.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin built automatic Neo4j Desktop discovery, designed the 24 typed operations, and separated read-only query behavior from lifecycle and administration actions.',
      roleOfGenAI: 'An AI client discovers and composes the explicit graph tools; Griffin defines the boundaries and keeps each operation visible rather than granting arbitrary shell access.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin built the Python CLI and MCP surface, automated the signed-in desktop app through accessibility and window messages, and constrained the bridge to read and reason.',
      roleOfGenAI: 'Microsoft 365 Copilot supplies work-grounded answers; a local MCP client can request and reuse that text while Griffin’s bridge controls the desktop interaction.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin designed the plaintext organization, writes and curates the engineering notes, and decides which context is reliable enough to reuse.',
      roleOfGenAI: 'Coding agents search the retained engineering memory to recover project context and reuse prior decisions; they do not replace Griffin’s curation or create hidden memory.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin configured the core and community plugins and the obsidian-git settings used by the local workspace.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin built the container setup, one-time PostgreSQL migration, filestore seed, health checks, and recovery controls around the copied application artifacts.',
      roleOfGenAI: 'Generative AI assisted the deployment and diagnostic scripting while Griffin controlled the snapshot, migration order, persistence, and recovery decisions.',
    ...
    defineProject({
      ...
      griffinsRole: 'Griffin built the gateway, session authentication, local and Eureka discovery modes, resources, prompts, 77 registered tools, and runtime write gates.',
      roleOfGenAI: 'The AI client chooses among named Teamcenter tools; Griffin designed the tool surface and operator-controlled authority levels around the real SOA sessions.',
  ...
  const EXPLORATION_GROUPS = Object.freeze([
    Object.freeze({
      ...
      summary: 'Two native-engine builds compare what Codex and Sol could produce under Griffin’s direction, with playable evidence and candid limits instead of invented scores.',
